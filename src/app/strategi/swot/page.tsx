...
import { Layers, Zap, Target, Shield, AlertTriangle, Save, Loader2, Plus, Trash2, CheckCircle2, Edit, RotateCw, FileText } from 'lucide-react';
...
export default function SWOTPage() {
    ...
    const handleExportPDF = () => {
        ...
        doc.text('LAPORAN ANALISIS SWOT', pageWidth / 2, pageHeight / 2 - 60, { align: 'center' });
        ...
        doc.setFontSize(13);
        doc.setFont('helvetica', 'bold');
        doc.text('A. Inventarisasi Faktor SWOT', 40, 140);
        ...
        const tableCats = [
            ...
            { key: 'ancaman', label: '4. FAKTOR ANCAMAN (THREATS)' },
        ] as const;
        ...
        tableCats.forEach((catInfo) => {
            ...
            let rowIdx = 1;
            const items = form[catInfo.key as SwotCategory];
            ...
            autoTable(doc, {
                startY: finalY + 22,
                head: [['No', 'Faktor Deskripsi', 'Bobot', 'Rating (Ranking)', 'Skor']],
                body: tableData,
                theme: 'grid',
                ...
                didDrawPage: (data) => {
                    ...
                    if (currentPage > contentPageStart) {
                        addHeader(doc, 'Laporan Analisis SWOT');
                    }
                }
            });
        ...
        doc.text('1. Inventarisasi Faktor SWOT (Internal & Eksternal)', 40, 140);
        ...
        doc.text('Pengelola Analisis Strategis', 60, finalY + 78);
        ...
        addFooter(doc);
        doc.save(`Laporan_SWOT_${year || 'Semua'}.pdf`);
    };
